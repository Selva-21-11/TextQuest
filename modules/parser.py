    ...
    for doc in documents:
        ...
        while i < len(lines):
            ...
            if section_match:
                ...
                marks_info = section_match.group(2).strip()
                current_section = f"Section {section_num} {marks_info}".strip()
                if current_part:
            ...
            # Detect question number and question text
            q_match = re.match(r'^(\(?\d+[a-zA-Z]?\)?)[\.\)]?\s*(.+)', line)
            if q_match:
                number = q_match.group(1).strip("().")
                question = q_match.group(2).strip()

                # Detect MCQ options (if present)
                options = []
                is_mcq = False
                while i + 1 < len(lines):
                    opt_line = lines[i + 1]
                    if re.match(r'^\(?[a-dA-D]\)?\s+.+', opt_line):
                        options.append(opt_line.strip())
                        is_mcq = True
                        i += 1
                    else:
                        break
                ...
                # Add to structured questions with MCQ flag
                question_data = {
                    "number": number,
                    "question": question,
                    "marks": estimate_marks(current_section),
                    "is_mcq": is_mcq
                }
